...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const bought = document.getElementById("buy");
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    bought.addEventListener("click", function (event) {
    const selectedSeats = document.querySelectorAll(".seat.selected");
    const totalSeats = selectedSeats.length;
    const totalCost = calculateTotalCost(selectedSeats);
        // Verifica si el clic proviene del botón con el ID "buy"
  ...
  buyButton.addEventListener("click", function () {
    ...
    if (totalSeats > 0) {
      window.location.href = "/html/comprar-entradas/3/comrprarEntradas-Fantasma3.html";
      totalCostInfo.textContent = `Asientos seleccionados: ${totalSeats} | Costo total: $${totalCost}`;
